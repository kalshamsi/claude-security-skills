# security-headers-audit Behavioral Test Session
...
Run 10 behavioral test prompts against the **security-headers-audit** skill, score each, and record results in `docs/test-report.md`.

## Critical Methodology Rules

1. **Subagents launch from the project root** — this is required so the subagent inherits the project's installed skills (including security-headers-audit). Do NOT change the working directory.
2. **No tool hints, no path hints, no bias** — give each subagent ONLY the prompt text below. Do NOT mention security-headers-audit, skill names, testing, scoring, or expected behavior in the subagent prompt.
3. **No security tools should be pre-installed** — this is a pure-analysis skill. Do NOT install observatory, securityheaders.com CLI, or any scanning tools before testing.
...
2. Read `skills/security-headers-audit/SKILL.md` — this is the workflow you're scoring against.
3. Read `docs/test-prompts.md` section 6 (security-headers-audit) — the Expected Behavior fields tell you what to look for.
4. Verify no security scanning tools are installed:
```bash
which semgrep && echo "WARNING: semgrep found" || echo "OK"
which observatory && echo "WARNING: observatory found" || echo "OK"
```
5. Verify fixture files exist:
```bash
ls tests/fixtures/security-headers-audit/
# Should show: express-app.js, nginx.conf, next.config.js, README.md
```
...
Score 3 if the skill proceeds directly to configuration analysis. Score 2 if it mentions tools but proceeds. Score 1 if it attempts installation then falls back. Score 0 if it blocks on missing tools.
...
| **Output Quality** | Freeform | Some structure, missing fields | Severity + CWE + OWASP, no code pairs | Full format + config pairs + summary table |
...
- **T-2** scores on **Workflow** dimension (does explicit /security-headers-audit invocation produce clean workflow entry?). Do NOT score T-2 on Triggering.
- **FI-1/FI-2** — this is a **pure-analysis skill**. Score 3 = begins config analysis immediately with zero install attempts. Any tool installation attempt is a skill quality issue.
- **Multi-layer analysis** — WA-2 and OQ prompts should ideally note header interactions across Express, Nginx, and Next.js layers (e.g., Nginx's proxy_hide_header stripping upstream headers).
...
2. **Update `docs/test-report.md`:**
   - Overwrite the security-headers-audit row with new scores and recalculate avg.
